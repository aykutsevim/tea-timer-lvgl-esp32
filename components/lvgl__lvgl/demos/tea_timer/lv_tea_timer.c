 ...
 *      INCLUDES
 *********************/
#include "lv_tea_timer.h"
#include <stdio.h> 

#if LV_USE_TEA_TIMER

#if LV_MEM_CUSTOM == 0 && LV_MEM_SIZE < (38ul * 1024ul)
    #error Insufficient memory for lv_tea_timer. Please set LV_MEM_SIZE to at least 38KB (38ul * 1024ul).  48KB is recommended.
#endif
...
typedef enum {
    ...
    TEA_NOT_READY,
    TEA_READY_FRESH,
    TEA_READY_PERFECT_STEEP,
    TEA_READY_OVER_STEEP,
    TEA_UNDRINKABLE
} tea_state_t;
...
void lv_tea_timer(void)
{
    ...
    tea_timer = lv_timer_create(tea_timer_cb, 100, NULL);


    color_changer_create(tv);
}

void lv_tea_timer_close(void)
{
...
static void profile_create(lv_obj_t * parent)
{
    ...
    lv_label_set_text(label, "Baslat");
    ...
    label = lv_label_create(sifirla_btn);
    lv_label_set_text(label, "Sifirla");
    lv_obj_center(label);
    ...
    if(disp_size == DISP_LARGE) {
        ...
        lv_obj_set_width(baslat_btn, 120);
        lv_obj_set_width(sifirla_btn, 120);
        ...
        lv_obj_set_grid_cell(baslat_btn, LV_GRID_ALIGN_START, 1, 1, LV_GRID_ALIGN_CENTER, 5, 1);
        lv_obj_set_grid_cell(sifirla_btn, LV_GRID_ALIGN_END, 3, 1, LV_GRID_ALIGN_CENTER, 5, 1);
    }
        ...
        lv_obj_set_grid_cell(baslat_btn, LV_GRID_ALIGN_STRETCH, 0, 2, LV_GRID_ALIGN_CENTER, 5, 1);
        lv_obj_set_grid_cell(sifirla_btn, LV_GRID_ALIGN_STRETCH, 0, 2, LV_GRID_ALIGN_CENTER, 6, 1);
